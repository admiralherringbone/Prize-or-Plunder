/**
 * Represents a complete Barque Rig assembly.
 * A barque has three masts: square-rigged fore and main masts, and a fore-and-aft rigged mizzenmast.
 */
class BarqueRig {
    /**
    ...
    getDurabilityContribution() {
        ...
        const sailLength = this.ship.shipLength / 3;
        totalDimension += sailLength;
        totalDimension += sailLength * (3 / 5);
     ...
     * The barque has a gaff sail on its mizzenmast.
    ...
    _drawMastBaseAndSails(ctx, zoomLevel, windDirection, mastConfig) {
        if (mastConfig.type === 'mizzen') {
            ...
            this._drawFullGaffSailAssemblyOnMizzen(ctx, zoomLevel, windDirection);
            ctx.restore();
        }
    ...
    /**
     * Draws a complete full gaff sail assembly on the mizzenmast.
     * @param {CanvasRenderingContext2D} ctx - The canvas rendering context.
     * @param {number} zoomLevel - The current camera zoom level.
     * @private
     */
    _drawFullGaffSailAssemblyOnMizzen(ctx, zoomLevel, windDirection) {
        // The mizzen gaff sail is a fixed proportion of the ship length.
        const sailLength = this.ship.shipLength / 3;
    ...
    // A future refactor could move these to shared assembly classes.
